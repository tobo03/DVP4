...
def alsoViewTTL(df, graph):
    #Prepping the dataframe
    df = df[['asin', 'also_view']]
    df = df[df['also_view'].astype(str) != '[]']
    df = df.explode('also_view')
    df = df.rename(columns = {'asin' : 'source', 'also_view' : 'target'})
    df['edge'] = len(df) * ['also_view']
