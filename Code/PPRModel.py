...
subsetCreation(g, withAuthor = False)
...
#Alpha, beta and S is defined
S = ['B01BHFC482', 'B00RQ7WSO6', 'B00I0MAH1W', 'B00I4SUJLA', 'B00YQGHFZ2']    
weights = {}
alpha = 0.2
beta = 1-alpha
S_len = len(S)
...
for i in range(100):
    ...
    if time.time() - t1 > 3600:
        ...
        with open(f"weights-final1-{i}.json", "w") as outfile:
        ...
        #Save each log
        text_file = open("log-final1.txt", "w")
...
with open("weights-final1-done.json", "w") as outfile:
    json.dump(weights, outfile)

#Save final log as .txt file.
text_file = open("log-final1-done.txt", "w")
n = text_file.write(log)
text_file.close()